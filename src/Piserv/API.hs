...
newtype BoardRev = BoardRev Int deriving Eq
newtype DigByte = DigByte Word8 deriving Eq
newtype GpioPinNumber = GpioPinNumber Int deriving Eq
newtype PhysPinNumber = PhysPinNumber Int deriving Eq
newtype PinResult = PinResult Int deriving Eq
newtype PinResultValue = PinResultValue Value deriving Eq
newtype PinValue = PinValue Value deriving Eq
newtype PudValue = PudValue Pud deriving Eq
newtype PwmMde = PwmMde PwmMode deriving Eq
newtype PwmVal = PwmVal Word16 deriving Eq
newtype PwmValClock = PwmValClock Word16 deriving Eq
newtype PwmValRange = PwmValRange Word16 deriving Eq
newtype WpiPinNumber = WpiPinNumber Int deriving Eq

instance Accept PlainText => MimeRender PlainText BoardRev where
  mimeRender _ (BoardRev n)  = toByteString n

instance Accept PlainText => MimeRender PlainText GpioPinNumber where
  mimeRender _ (GpioPinNumber n)  = toByteString n

instance Accept PlainText => MimeRender PlainText Health where
  mimeRender _ _  = "OK - PISERVER RUNNING"

instance Accept PlainText => MimeRender PlainText PinResultValue where
  mimeRender _ (PinResultValue LOW)  = "low"
  mimeRender _ (PinResultValue HIGH) = "high"
...
instance ToSample BoardRev where
  toSamples _ = singleSample (BoardRev 2)
...
instance ToSample PinResultValue where
  toSamples _ =
    [ ("When the value is low", (PinResultValue LOW))
    , ("When the value is high", (PinResultValue HIGH)) ]
...
type PIAPI =
  ...
  :<|> "digitalread" :> "wpi" :> Capture "wpiPinNumber" WpiPinNumber :> Get '[PlainText] PinResultValue
  :<|> "digitalread" :> "gpio" :> Capture "gpioPinNumber" GpioPinNumber :> Get '[PlainText] PinResultValue
  :<|> "digitalread" :> "phys" :> Capture "physPinNumber" PhysPinNumber :> Get '[PlainText] PinResultValue
  ...
  :<|> "piboardrev" :> Get '[PlainText] BoardRev
  :<|> "pintobcmgpio" :> "wpi" :> Capture "wpiPinNumber" WpiPinNumber :> Get '[PlainText] GpioPinNumber
  :<|> "pintobcmgpio" :> "gpio" :> Capture "gpioPinNumber" GpioPinNumber :> Get '[PlainText] GpioPinNumber
  :<|> "pintobcmgpio" :> "phys" :> Capture "physPinNumber" PhysPinNumber :> Get '[PlainText] GpioPinNumber
